...
import '../../../cubit/postInfo_cubit/comment_cubit/comment_like_cubit/comment_likes_cubit.dart';
import '../../../cubit/postInfo_cubit/comment_cubit/reply_cubit/reply_info_cubit.dart';
import '../../../cubit/postInfo_cubit/comment_cubit/reply_cubit/reply_like_cubit/reply_like_cubit.dart';
...
class _CommentInfoState extends State<CommentInfo> {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        children: [
          rowOfCommentator(context, isLiked, widget.commentInfo.comment),
  ...
  BlocBuilder<ReplyInfoCubit, ReplyInfoState> showReplies(
    ...
    return BlocBuilder<ReplyInfoCubit, ReplyInfoState>(
        ...
        buildWhen: (previous, current) {
          if (previous != current && (current is ReplyInfoLoaded)) {
            return true;
          }
        ...
        builder: (context, state) {
          ...
          } else if (state is ReplyInfoFailure) {
    ...
    return GestureDetector(
      onTap: () {
        Go(context).push(
            page: WhichProfilePage(
                userId: widget.commentInfo.whoCommentInfo?.userId ??
                    widget.commentInfo.whoCommentId));
      },
      child: Text.rich(
        TextSpan(
          children: <TextSpan>[
    ...
    return GestureDetector(
      ...
      onTap: () {
        setState(() {
          if (isLiked) {
            if (widget.isThatReply) {
              BlocProvider.of<ReplyLikeCubit>(context).removeLikeOnThisReply(
            ...
            if (widget.isThatReply) {
              BlocProvider.of<ReplyLikeCubit>(context).putLikeOnThisReply(
                replyId: widget.commentInfo.commentUid,
                myPersonalId: myPersonalId,
              );
            } else {
  ...
  CustomNotification createNotification(Comment commentInfo) {
    ...
    return CustomNotification(
      text: " liked your comment: ${commentInfo.comment}",
      postId: postInfo.postUid,
