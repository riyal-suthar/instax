...
import 'package:instax/presentation/pages/profile/widgets/profile_grid_view.dart';
import 'package:instax/presentation/pages/time_line/widgets/read_more_text.dart';
...
import '../../../widgets/popup_widgets/web/follow_card.dart';
import '../followers_info_page.dart';
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Widget tapBar() {
    return BlocBuilder<PostCubit, PostState>(
      bloc: PostCubit.get(context)
        ..getPostsInfo(
            postIds: widget.userInfo.value.posts,
      ...
      buildWhen: (previous, current) {
        ...
        return previous != current &&
            ((current is MyPersonalPostsLoaded && widget.isThatMyPersonalId) ||
                (current is PostsInfoLoaded && !widget.isThatMyPersonalId));
      },
      builder: (BuildContext context, PostState state) {
        if (state is MyPersonalPostsLoaded && widget.isThatMyPersonalId) {
          return columnOfWidgets(state.postsInfo);
        } else if (state is PostsInfoLoaded && !widget.isThatMyPersonalId) {
          return columnOfWidgets(state.postsInfo);
        } else {
          return isThatMobile
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                if(userInfo.bioLinks!.isNotEmpty) TextButton(
                    onPressed: ()async{
                      if(userInfo.bioLinks!.length > 1){
                        userBioLinkBottomSheet(userInfo.bioLinks!);
                      }else{
                    ...
                    child: Row(children: [
                      ...
                      Text(userInfo.bioLinks![0].toString())
                    ],)),
                const SizedBox(height: 10),
                Row(
  ...
  Future<void> userBioLinkBottomSheet(List bioLinks,{bool createNewData = true}) {
    return showModalBottomSheet<void>(
      context: context,
      builder: (BuildContext context) => CustomBottomSheet(
        bodyText: bodyTextOfBottomSheet(bioLinks, createNewData),
  ...
  Padding bodyTextOfBottomSheet(List bioLinks, bool createNewData) {
    return Padding(
      padding: const EdgeInsetsDirectional.only(start: 20.0),
      child: Column(
        children: bioLinks.map((e) => Text(e.toString())).toList(),
      ),
    );
  }
