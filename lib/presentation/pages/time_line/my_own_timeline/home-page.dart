...
import '../../../cubit/postInfo_cubit/specific_users_posts_cubit.dart';
import '../../../cubit/story_cubit/story_cubit.dart';
import '../../../cubit/user_info/user_info_cubit.dart';
import '../../../custom_packages/in_view_notifier/in_view_notifier_list.dart';
import '../../../custom_packages/in_view_notifier/in_view_notifier_widget.dart';
...
import '../../story/story_for_mobile.dart';
import '../../story/story_for_web.dart';
import '../widgets/all_catch_up_icon.dart';
import '../widgets/image_of_post_for_timeline.dart';
...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  Future<void> _getData(int index) async {
    ...
    await postCubit
        .getPostsInfo(
            postIds: postsIds, isThatMyPosts: true, lengthOfCurrentList: index)
        .then((value) {
  ...
  void initState() {
    ...
    if (isThatMobile) {
      ...
      // WidgetsBinding.instance.addPostFrameCallback((timeStamp) async{ await NotificationService() })
    }
    super.initState();
  }
    ...
    return ValueListenableBuilder(
      ...
      builder: (context, bool value, child) =>
        ...
        buildWhen: (previous, current) {
          if (value && current is MyPersonalPostsLoaded) {
            reLoadData.value = false;
            return true;
          }
          if (value) {
          ...
          if (previous != current && current is MyPersonalPostsLoaded) {
            return true;
          }
        ...
        builder: (BuildContext context, PostState state) {
          if (state is MyPersonalPostsLoaded) {
  ...
  Widget storiesLines(double bodyHeight) {
    ...
    return ValueListenableBuilder(
      ...
      builder: (context, bool value, child) =>
        ...
        buildWhen: (previous, current) {
          if (value && current is StoriesInfoLoaded) {
            reLoadData.value = false;
            return true;
          }
          if (value) {
          ...
          if (previous != current && current is StoriesInfoLoaded) {
            return true;
          }
          if (previous != current && current is StoryFailed) {
            return true;
          }
          return false;
        },
        builder: (context, state) {
          if (state is StoriesInfoLoaded) {
